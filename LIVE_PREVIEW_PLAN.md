...
Studio wiring + Sanity CORS done. **INTERIM DECISION (2026-06-02, R-18): the
custom-domain + Access path needs an apex DNS migration with real blast radius (the
pre-built Cloudflare `sasinfra.com` zone is a STALE snapshot — see R-18), so for now
the preview Worker runs WITHOUT Cloudflare Access on the `*.workers.dev` host.**
Removing Access makes R-15 moot (no framed login, no third-party cookie); mitigations
are the obscure URL + `X-Robots-Tag: noindex`. Shipped 2026-06-02: Access removed,
noindex header (frontend `a1dc62e`), Studio var reverted to
`https://sasinfra-frontend-preview.sas-infra.workers.dev` + Studio redeployed
(`workflow_dispatch` added, sanity-cms `260de09`). 🔴 **Re-add Access on
`preview.sasinfra.com` before any public/prod posture — it is the trust boundary.**
· **Rev:** v6 (2026-06-02; R-18 interim no-Access pivot supersedes the v5
custom-domain-now plan) · **Architecture:** A (static
...
| **R-18** | The R-15/R-16 custom-domain + Access path requires making Cloudflare authoritative for `*.sasinfra.com`, i.e. an apex DNS migration. A `dig` audit (2026-06-02) found the **pre-built Cloudflare `sasinfra.com` Full zone (Pro) is a STALE snapshot of the old S3/CloudFront era**: `cms` + `admin` records **absent** (would go down), `staging` → old AWS CloudFront IPs (not the Pages site), apex/`www` → different origin than the live Hostinger, CAA missing `amazon.com`, one `google-site-verification` TXT missing. Email (MX→Google, SPF, DKIM `google._domainkey`, DMARC) **is** mirrored and safe. Activating that zone (flip NS at GoDaddy) without reconciling those records breaks live services — too much blast radius for an internal editor preview right now. | **Interim decision (Krishna, 2026-06-02): run the preview Worker WITHOUT Cloudflare Access** on the `sasinfra-frontend-preview.sas-infra.workers.dev` host until prod launch. Removing the Access app makes R-15 moot (the iframe loads the Worker directly — no framed login, no third-party cookie) and **avoids the DNS migration entirely**. Mitigations: drafts are served only on the obscure workers.dev URL + `X-Robots-Tag: noindex` on every preview response (frontend `a1dc62e`, preview-only middleware); the read token stays a server-side Worker secret (never client-exposed) — only draft *content* is reachable. Studio `SANITY_STUDIO_PREVIEW_URL` reverted to the workers.dev host + Studio redeployed (`workflow_dispatch` added to `deploy.yml`, sanity-cms `260de09`). 🔴 **HARD GATE before any public/prod posture: re-add Access on the same-parent-domain `preview.sasinfra.com` (the R-15/R-16 pivot, which by then is natural if the apex is moving to Cloudflare for launch) — Access is the actual trust boundary, not the forgeable cookie.** |

### Round 6 — the actual G7 eyeball, post-R-18 (2026-06-03): "works for me, not others"
First real in-browser test of the no-Access workers.dev posture, on a colleague's machine
(screen-share). Two stacked, independent failures behind the same "Unable to connect to
visual editing" symptom — **neither is the CORS error in the console** (that's the Studio
fetching its own `/static/manifest.webmanifest`, which hits Access and is redirected to the
login page → a benign CORS failure on every load; the `wss://…api.sanity.io/socket` warning
is the Studio's content listener, also unrelated). The preview Worker itself was verified
100% healthy (drafts render, islands hydrate, `<sanity-visual-editing>` mounts, 0 console
errors). Both fixes shipped + verified live the same day.
| # | Finding | Resolution |
|---|---|---|
| **R-19** | 🔴 **Stale Studio bundle pinned the dead preview URL ("works for me, not others").** The Studio's `_headers` cached hashed `/static/*` bundles `immutable` for a year but set **no `Cache-Control` on `index.html`**. Anyone who loaded the Studio between 2026-06-01 07:31 and 06-02 05:49 (when the repo var still baked `preview.sasinfra.com`) kept a cached `index.html` referencing the old immutable bundle → its Presentation iframe pointed at `preview.sasinfra.com`, which **does not resolve** (custom domain never provisioned, R-18) → "Unable to connect." Krishna had reloaded fresh (workers.dev); colleagues hadn't. NB: for Cloudflare **Pages**, a redeploy *is* the edge-cache invalidation — there is no separate purgeable zone cache for the SSL-for-SaaS custom domain, and the account token has no purge scope; the stale copy is purely in the browser. | **Fixed (sanity-cms `1d31530`):** `_headers` now sets `Cache-Control: no-cache` on `/*` (HTML always revalidates) while `/static/*` stays `immutable` (later rule wins). Future loads can't pin a stale bundle. Already-stuck browsers need a one-time hard refresh / **Clear site data** / incognito (the no-cache header only fixes *future* staleness). |
| **R-20** | 🔴 **Visual editing was gated on the perspective, not the session.** The Presentation tool opens the iframe in the **`published`** perspective by default (`?perspective=published`), setting `sanity-preview-perspective=published`. The preview runtime gated on `isPreviewing()` = `perspective !== 'published'`, so a genuine preview session in published perspective mounted **no VE island, no stega, no source map** → the Studio had nothing to handshake with → "Unable to connect to visual editing." Editors only got overlays if they manually switched to Drafts (the real "works for me": Krishna's Studio remembered Drafts; fresh users got the Published default). Confirmed on the live Worker: a `published` cookie → 0 islands / 0 stega; after fix → 2 islands / 26.8k stega. | **Fixed (frontend `8a35132`, Codex-reviewed):** gate the runtime on the **session** — the perspective cookie's *presence* (any value) — not the perspective. ALS now carries `{ perspective, preview }`; new `isPreviewSession()` drives the island + stega + read token + the `siteSettings`/`projectSeo` module-cache bypass (Codex P1: keeps the shared published cache from mixing clean and stega-bearing copies). `getPerspective()` still drives *which* content `loadQuery` fetches; removed the now-unused `isPreviewing()`. Preview-build-only (dead-code-eliminated in prod — both builds verified; public visitor with no cookie stays clean). |
| **R-21** | ⏳ **Still-open contingency.** R-19 + R-20 fix the stale-bundle and published-perspective walls. If a colleague *still* sees "unable to connect" after a fresh reload, the remaining cause is the **third-party draft cookie** being blocked: the `sanity-preview-perspective` cookie is set on `workers.dev` (third-party to the `sasinfra.com` Studio parent), so a browser with strict 3rd-party-cookie blocking never sends it back in the iframe → no session → no island. This is the same registrable-domain boundary as R-15/R-16. | **No code fix possible on workers.dev** — resolved only by the deferred move to **`preview.sasinfra.com`** (same registrable domain → first-party cookie), i.e. the R-16 runbook in §13 G7. Default Chrome (2026) still allows 3rd-party cookies, so most editors are unaffected for now; this is the last reason to do the custom-domain move (alongside re-adding Access). |
